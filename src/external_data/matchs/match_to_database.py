...
for i, row in df.iterrows():
    ...
    try:
        ...
        (ID,date_day,date_month,date_year,date_hour,date_minute,map_name,length_hour,length_minute,length_second,participants,winner,cloudy_ver) 
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) """

        data = (i,row['date_day'], row['date_months'], row['date_year'], row['date_hour'], row['date_minute'], row['map_name'], row['length_hour'],
